    ...
    try {
      ...
      if (!cycleResult.executed || !cycleResult.proposals || cycleResult.proposals === 0) {
        return this.reject(runId, runVersion.version, 'proposal',
          cycleResult.reason || 'No proposals generated', startTime);
      }
      // Use the first proposal from the cycle (cast via any to avoid strict type mismatch)
      const rawProposal = (cycleResult as any).proposalList?.[0];
      if (rawProposal) {
        proposal = rawProposal as InternalProposal;
      } else {
        // Create a synthetic proposal from cycle result for sandbox testing
        proposal = {
          id: `${runId}-synthetic`,
          summary: `Auto-optimization from cycle ${this.config.cycle}`,
          targetFile: 'server/mother/core-orchestrator.ts',
          changeType: 'refactor',
          priority: 'medium',
          estimatedImpact: cycleResult.convergenceScore || 0.85,
          hash: `${runId}-hash`,
          createdAt: new Date(),
          codeDiff: '// DGM auto-optimization placeholder',
          description: `Auto-optimization proposal from cycle ${this.config.cycle}`,
          rationale: 'Generated by DGM cycle3 convergence',
        };
      }
      log.info(`[${runId}] Proposal generated: ${proposal.id} changeType=${proposal.changeType}`);
    } catch (err: any) {
