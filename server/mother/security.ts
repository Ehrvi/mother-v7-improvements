...
export class ThreatDetector {
  ...
  analyze(input: {
    ...
    if (input.ipAddress && input.failedAttempts !== undefined) {
      results.push(
        this.detectBruteForce(input.ipAddress, input.failedAttempts, 60000)
      );
    }

    if (
      input.requestRate !== undefined &&
      input.historicalMean !== undefined &&
      input.historicalStdDev !== undefined
    ) {
      results.push(
        this.detectAnomalous(
          input.requestRate,
          input.historicalMean,
...
export function createSecurityMiddleware() {
  return {
     ...
     */
    checkThreats(
      input: Parameters<ThreatDetector["analyze"]>[0]
    ): ThreatDetectionResult[] {
    ...
    recordMetrics(
      ...
      globalSecurityMetrics.recordRequest(
        blocked,
        threatDetected,
        responseTime
      );
