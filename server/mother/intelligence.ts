...
export function assessComplexity(query: string): ComplexityAssessment {
  ...
  const multiStepIndicators = [
    ...
    'explain how', 'explain why', 'describe the process'
  ];
  
  const multiStepCount = multiStepIndicators.filter(ind => queryLower.includes(ind)).length;
  if (multiStepCount >= 2) {
    complexityScore += 0.2;
  ...
  const codeIndicators = ['code', 'function', 'class', 'method', 'variable', 'debug', 'error'];
