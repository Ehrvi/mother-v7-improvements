...
function checkCompleteness(
  ...
  if (isQuestion) {
    ...
    const hasAnswer =
      response.length > 100 && !/^(i don't know|sorry)/i.test(response);
    if (!hasAnswer) {
      score -= 15;
...
async function checkRelevance(
  ...
  const relevanceRatio =
    queryTerms.length > 0 ? matchedTerms.length / queryTerms.length : 1;

  // Adjust thresholds for keyword matching (more lenient than semantic)
  if (relevanceRatio < 0.2) {
    score = 60;
    issues.push(
      `Low term overlap with query (${(relevanceRatio * 100).toFixed(1)}%)`
    );
  } else if (relevanceRatio < 0.4) {
    score = 80;
    issues.push(
      `Moderate term overlap with query (${(relevanceRatio * 100).toFixed(1)}%)`
    );
  ...
  logger.info(
    `[Guardian] Keyword relevance: ${(relevanceRatio * 100).toFixed(1)}%, Score: ${score}`
  );
...
function checkSafety(
  ...
  const ethicalPatterns = [/discriminat/i, /bias/i, /unfair/i];
