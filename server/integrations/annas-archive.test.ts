...
describe('Anna\'s Archive Integration', () => {
  ...
  describe('download', () => {
    ...
    it('should throw error on invalid URL', async () => {
      const invalidUrl = 'https://invalid-url-that-does-not-exist.com/paper.pdf';
      const filename = 'test.pdf';

      await expect(annasArchive.download(invalidUrl, filename)).rejects.toThrow();
    });
  });
  ...
  describe('addToKnowledgeBase', () => {
    it('should add paper to knowledge base', async () => {
      ...
      const conceptId = await annasArchive.addToKnowledgeBase(mockPaper, mockContent);
    ...
    it('should include metadata in knowledge base entry', async () => {
      ...
      const conceptId = await annasArchive.addToKnowledgeBase(mockPaper, 'Content');
  ...
  describe('integration with Knowledge Base', () => {
    ...
    it('should enable semantic search of papers', async () => {
      ...
      const mockPaper = {
        ...
        year: 2017,
        source: 'NeurIPS',
        abstract: 'Attention is all you need - introducing the Transformer model',
        doi: '10.5555/TEST.2017.001'
      };

      await annasArchive.addToKnowledgeBase(mockPaper, 'Full paper content about transformers and attention mechanisms');
