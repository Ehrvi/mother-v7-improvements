 ...
 */
export function getMotherQueue(): Queue<
  MotherQueryJob,
  MotherQueryResult
> | null {
  ...
  try {
    ...
    motherQueue = new Queue<MotherQueryJob, MotherQueryResult>(
      "mother-queries",
      {
        connection: {
          host: redisHost,
...
export async function enqueueQuery(
  ...
  try {
    ...
    logger.info(
      `Query enqueued: ${job.queryHash.substring(0, 8)} (priority: ${priority || 0})`
    );
    return addedJob;
  } catch (error) {
  ...
  try {
    ...
    motherWorker = new Worker<MotherQueryJob, MotherQueryResult>(
      ...
      async (job: Job<MotherQueryJob, MotherQueryResult>) => {
        ...
        logger.info(
          `Processing job ${job.id}: ${job.data.query.substring(0, 50)}...`
        );
...
export async function cleanOldJobs(): Promise<{
  ...
  try {
    ...
    logger.info(
      `Cleaned ${completedCleaned.length} completed jobs, ${failedCleaned.length} failed jobs`
    );
