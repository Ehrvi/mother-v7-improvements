# TODO-ROADMAP V37 — MOTHER v90.0 — Ciclo C208 — Council R5 CONCLUÍDO

**Versão:** TODO-ROADMAP V37
**Sistema:** MOTHER v90.0
**Ciclo:** C208 — Council R5 CONCLUÍDO
**Data:** 2026-03-09
**Score Atual:** 98.5/100 (estimado pós-C208)
...
- [x] **C208-1: NC-UX-005 FIXED — Font-size ≥10px** — 6 arquivos corrigidos
  - `client/src/pages/Home.tsx` — 23 ocorrências text-[8px]/text-[9px] → text-[10px]/text-[11px]
  - `client/src/components/RightPanel.tsx` — 18 ocorrências
  - `client/src/components/ExpandableSidebar.tsx` — 4 ocorrências
  - `client/src/components/MotherMonitor.tsx` — 3 ocorrências
  - `client/src/components/DGMPanel.tsx` — 2 ocorrências
  - `client/src/pages/DgmLineage.tsx` — 2 ocorrências
...
- [x] **C208-2: NC-UX-006 FIXED — Aria-labels WCAG** — `client/src/pages/Home.tsx`
  - `aria-label="Enviar mensagem"` no botão de envio
  - `aria-label="Campo de mensagem para MOTHER"` no textarea
  - `aria-label="Anexar arquivo"` no botão de anexo
  - `role="log" aria-label="Histórico de conversa" aria-live="polite"` no container de mensagens
  - `role="heading" aria-level={2|3}` em cabeçalhos de seção
...
- [x] **C208-3: NC-SEC-001 FIXED — Race Condition runMigrations** — `server/_core/production-entry.ts`
  - `runMigrations()` movido para ANTES de `app.listen()`
  - Elimina race condition onde Cloud Run envia tráfego antes das migrações completarem
...
- [x] **C208-4: NC-INFRA-005 DOCUMENTADO — Rate Limiter In-Memory** — `server/_core/rate-limiter.ts`
  - Comentário NC-INFRA-005 adicionado explicando limitação do MemoryStore em Cloud Run multi-instância
  - Correção planejada Sprint 10: Redis-backed store (rate-limit-redis)
  - Base científica: OWASP API4:2023 + Google Cloud Run docs (2024)

- [x] **C208-5: inject-knowledge-c208-council-audit.cjs** — 15 entradas BD inseridas
  - BD: 142 → **157** (+15 entradas C208)
  - Entradas: NC-UX-005/006/007, NC-SEC-001/002/003, NC-INFRA-005, NC-ARCH-004/005, NC-PERF-001/002, Council R5 síntese, MOTHER v90.0, WCAG 2.1, Nygard 2007, OWASP API4:2023, React Error Boundaries, CSP

- [x] **C208-6: Versão v90.0** — `server/mother/core.ts` MOTHER_VERSION = 'v90.0' + package.json 90.0.0 + cloudbuild.yaml MOTHER_CYCLE=208
...
- [ ] **C208-10: Git commit C208-R001** — `feat(c208-r001): v90.0 + Council R5 UX/UI fixes + NC-SEC-001 race condition + 15 BD entries`

- [ ] **C208-11: Cloud Build deploy** — build C208-R001 → Cloud Run produção v90.0
...
**Critério Científico:** Google A2A Protocol v2 (2025) + ISO 13374-1:2003 + OWASP A03:2021 CSP

### Entregáveis Sprint 9 C208

- [ ] **C208-S9-1: A2A Protocol v2** — `server/mother/a2a-server-v2.ts`
  - Atualizar de A2A v1 para Google A2A Protocol v2 (2025)
  - Agent Card v2: capabilities, skills, authentication
  - NC-A2A-001 FIX
  - Base científica: Google A2A Protocol v2 (2025) — https://google.github.io/A2A

- [ ] **C208-S9-2: Multi-tenant SHMS** — `server/shms/shms-multitenant.ts`
  - Isolamento por tenant: cada cliente tem suas próprias estruturas e sensores
  - Row-level security via tenantId em todas as queries
  - NC-MULTI-001 FIX
  - Base científica: ISO 13374-1:2003 §4.2 + OWASP A01:2021 Broken Access Control

- [ ] **C208-S9-3: Dashboard SHMS v3** — `client/src/pages/SHMSDashboardV3.tsx`
  - Visualização Digital Twin em tempo real (WebSocket)
  - Gráficos: LSTM predictions vs. readings, anomaly timeline, health index
  - NC-SHMS-004 FIX
  - Base científica: Grieves (2014) Digital Twin + ISO 13374-1:2003 + D3.js

- [ ] **C208-S9-4: NC-SEC-002 — CSP Headers** — `server/_core/production-entry.ts`
  - Adicionar Content-Security-Policy headers via helmet
  - `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'`
  - Base científica: OWASP A03:2021 Injection + MDN CSP (2024)

- [ ] **C208-S9-5: NC-SEC-003 — Sanitização de Logs** — `server/mother/core.ts`, `server/_core/production-entry.ts`
  - Remover/mascarar API keys e tokens de logs
  - Padrão: `key.substring(0, 8) + '...'` para logs de debug
  - Base científica: OWASP A09:2021 Security Logging and Monitoring Failures

- [ ] **C208-S9-6: HippoRAG2 C208** — indexar papers Council R5 C208
  - Papers: WCAG 2.1 (W3C 2018), Nygard (2007) Release It!, OWASP API Security Top 10 2023, React Error Boundaries (2024), Google A2A Protocol v2 (2025)
  - Base científica: HippoRAG2 (arXiv:2502.14902)

- [ ] **C208-S9-7: inject-knowledge-c208-sprint9.cjs** — 15 entradas BD

- [ ] **C208-S9-8: Versão v91.0** — `server/mother/core.ts` MOTHER_VERSION = 'v91.0' + package.json + cloudbuild.yaml

- [ ] **C208-S9-9: AWAKE V290** — R87-R91 + PASSO 28

- [ ] **C208-S9-10: TODO-ROADMAP V38** — Sprint 9 concluído + Sprint 10 planejado

- [ ] **C208-S9-11: Git commit C208-S9-R001** — `feat(c208-s9-r001): v91.0 + A2A v2 + Multi-tenant SHMS + Dashboard v3 + CSP`
...
**Objetivo:** Refatoração UX (inline styles → CSS classes), Error Boundaries, Performance (useMemo/useCallback), Redis rate limiter
**Score Meta:** 99.0/100 → **99.5/100** (+0.5 ponto)
**Critério Científico:** Martin (2008) Clean Code + React docs (2024) + OWASP API4:2023
...
| NC-SEC-002 | Ausência de Content Security Policy (CSP) headers | ALTA | Sprint 9 C208 |
| NC-SEC-003 | Tokens de API expostos em logs | ALTA | Sprint 9 C208 |
...
**TODO-ROADMAP V37 — MOTHER v90.0 — Ciclo C208 — Council R5 CONCLUÍDO**
**Score: 98.5/100 ✅ | Threshold R33 ATINGIDO | Próximo: Sprint 9 C208 — A2A Protocol v2 + Multi-tenant SHMS + Dashboard SHMS v3**
